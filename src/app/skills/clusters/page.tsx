...
export default async function SkillClustersPage() {
  const clusters = await Promise.all(TOPICS.map(async (topic)=>({topic, content: await getTopicContent(topic.slug)})));
  return <main className="min-h-screen bg-white py-12"><div className="mx-auto max-w-7xl px-6 lg:px-8">
    ...
    <div className="mt-12 grid gap-6 lg:grid-cols-2">{clusters.map(({topic,content})=><section key={topic.slug} className="rounded-3xl border border-slate-200 p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-bold text-slate-950">{topic.name}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{topic.description}</p></div><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">{content.skills.length} skills</span></div><p className="mt-4 text-sm font-medium text-slate-700">{topic.startingPoint}</p><div className="mt-5 flex flex-wrap gap-2">{content.skills.slice(0,10).map((skill)=><Link key={skill.slug} href={`/skills/${skill.slug}`} className="rounded-full border border-slate-200 px-3 py-1.5 text-sm font-semibold text-slate-700 hover:border-blue-300 hover:text-blue-700">{skill.name}</Link>)}</div><div className="mt-6 flex gap-5 border-t border-slate-100 pt-5 text-sm font-bold"><Link href={`/topics/${topic.slug}`} className="text-blue-700">Explore full cluster →</Link>{content.paths[0]&&<Link href={`/paths/${content.paths[0].slug}`} className="text-slate-700">Follow a path →</Link>}</div></section>)}</div>
  </div></main>;
}
