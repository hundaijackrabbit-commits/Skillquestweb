...
export function SkillGuideOrb() {
  ...
  const startConversation = useCallback(async () => {
    if (!['idle', 'error'].includes(state)) return; setError(null); setState('connecting');
    try {
      ...
      socket.onmessage = async (event) => { let message: { event?: string; stream_id?: string; media?: { payload?: string }; message?: string }; try { message = JSON.parse(event.data); } catch { return; }
  ...
  }, [cleanup, clearPlayback, pathname, playPcm, startMicrophone, state]);
